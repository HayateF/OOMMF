/* modified to spinhevolve.cc from spintevolve.cc */
/* Spin Hall torque is achieved by adding field H_SLT to H,
   where H_SLT = tau m^sigma, tau = hbar theta_SH j / 2e M_s t_F , sigma = j_5^z_5,
   theta_SH is spin Hall angle, j is current density, t_F is thickness of sample,
   j_5 is unit vector whose direction is along current density, 
   z_5 is unit z-direction vector.
 */
/* In SpinHEvolve, tau and current_direction must be given as a parameter.
   current_direction is 1. or -1. corresponding to +x or -x respectively.	 */
...
OXS_EXT_REGISTER(Anv_SpinHEvolve);

/* End includes */

// Constructor
Anv_SpinHEvolve::Anv_SpinHEvolve(
    ...
    rkstep_ptr(NULL)
{
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Anv_SpinHEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Anv_SpinHEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Anv_SpinHEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Anv_SpinHEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Anv_SpinHEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_Ext::Error(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Anv_SpinHEvolve::UpdateDerivedOutputs);
  ...
  ave_u_output.Setup(this,InstanceName(),"average u","m/s",0,
		 &Anv_SpinHEvolve::UpdateSpinTorqueOutputs);
...
void Anv_SpinHEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Anv_SpinHEvolve::UpdateMeshArrays"
...
OC_REAL8m Anv_SpinHEvolve::EvaluateuProfileScript
(OC_UINT4m stage,
...
void Anv_SpinHEvolve::Calculate_dm_dt
...
{ // Imports: state, mxH_, pE_pt_
  ...
  for(i=0;i<size;i++) {
	...
	dm_dt_[i].Set(0.0,0.0,0.0);
			...
			ThreeVector H_SLT(0.,0.,0.);	//modification 4
			if(x>0) {
				j = i-1;
				if(Ms_[j]!=0.0) {
			...
			scratch3 = H_SLT;
			scratch3 ^= spin_[i];
			scratch3 *= cell_mgamma;
			dm_dt_[i] += scratch3; // dm_dt= -gamma_LL*m^H-alpha*gamma_LL*m^(m^H) -(1+alpha*beta)/(1+alpha^2)*u*pm_px +(beta-alpha)/(1+alpha^2)*u*m^pm_px -gamma_LL*m^H_SLT
...
void Anv_SpinHEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinHEvolve::CheckCache: Invalid data cache.");
  }
}


void
Anv_SpinHEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Anv_SpinHEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Anv_SpinHEvolve::NegotiateTimeStep() method.
}


void Anv_SpinHEvolve::UpdateTimeFields
...
void Anv_SpinHEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinHEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
			    timestep_lower_bound)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinHEvolve::NegotiateTimeStep: "
...
void Anv_SpinHEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_Ext::Error(this,
			 "Anv_SpinHEvolve::TakeRungeKuttaStep2:"
...
void Anv_SpinHEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinHEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_Ext::Error(this,
		 "Anv_SpinHEvolve::RungeKuttaFehlbergBase54:"
...
void Anv_SpinHEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinHEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinHEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Anv_SpinHEvolve::MaxDiff
...
void Anv_SpinHEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Anv_SpinHEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_Ext::Error(this,
	   "Anv_SpinHEvolve::Step; Programming error:"
  ...
  if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
    throw Oxs_Ext::Error(this,"Anv_SpinHEvolve::Step:"
...
void Anv_SpinHEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Anv_SpinHEvolve Oxs_ScalarOutput's to
...
void Anv_SpinHEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
